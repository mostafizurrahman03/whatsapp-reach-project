...
<div id="solutions" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="flex items-start sm:items-center justify-between gap-6 mb-10">
        <div>
            <span class="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-[#0F7B71]/10 text-[#0F7B71] text-xs font-semibold ring-1 ring-[#0F7B71]/20">Our Solutions</span>
            <h2 class="mt-3 text-3xl sm:text-4xl font-extrabold tracking-tight text-gray-900">Solutions</h2>
            <p class="mt-2 text-base text-gray-600 max-w-2xl">Choose a solution that fits your organisation — from startups to enterprises.</p>
        </div>
        <div class="hidden sm:flex items-center gap-2">
            <a href="#" class="text-sm px-4 py-2 rounded-lg border border-[#0F7B71]/30 text-[#0F7B71] hover:bg-[#0F7B71]/10 hover:border-[#0F7B71]/50 transition-colors">Explore all</a>
        </div>
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($items as $item)
            <div class="group rounded-2xl border border-gray-200/70 bg-white/70 backdrop-blur-sm p-5 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
                    <div class="flex items-start gap-4">
                    <div class="flex-shrink-0">
                        {{-- Simple icon switch (inline SVG) --}}
                        @switch($item["icon"] ?? 'briefcase')
                            @case('chat')
                                <div class="bg-[#0F7B71]/10 p-2.5 rounded-xl ring-1 ring-[#0F7B71]/15">
                                ...
                                @break
                            @case('chip')
                                <svg class="w-10 h-10 p-2.5 bg-[#0F7B71]/10 text-[#0F7B71] rounded-xl ring-1 ring-[#0F7B71]/15" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="mt-4">
                            <a href="#" class="inline-flex items-center gap-2 text-sm font-semibold px-3 py-2 rounded-lg text-[#0F7B71] bg-[#0F7B71]/5 hover:bg-[#0F7B71]/10 hover:gap-2.5 transition-all">
                                Learn more
